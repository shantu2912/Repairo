    ...
    Alpine.data('trackingApp', () => ({
        ...
        billSubtotal: 0,        // sum of all itemized rows (fixed services + other-service quote)
        billPlatformFee: 0,     // flat platform fee, only for non-inspection bookings
        billGrandTotal: 0,      // total value of everything billed
        billInspectionFee: 299, // inspection/visiting fee paid upfront
        billQuoteAmount: 0,     // technician's final quote for "Other Service"
        billAdvancePaid: 0,     // amount paid at booking time
        billBalancePaid: 0,     // amount paid after job completion
        billRefundDue: 0,       // if quote < inspection fee already paid
        billAmountInWords: '',
        isPrinting: false,
                        ...
                        if (payload.new) {
                            ...
                            if (payload.new.quote_status !== undefined) {
                                ...
                                if (payload.new.quote_status === 'approved') {
                                    ...
                                    // Refresh job data to update bill amounts
                                    this.refreshJobData();
                                }
        ...
        updateBillAmounts(job) {
            // Full bill totals are recomputed in openBillModal() right before the
            // invoice is shown, so this just keeps a fresh copy of the job record.
            this.fullJobData = job;
        },
    ...
    try {
        ...
        // ✅ FIXED: Use services_selected instead of device
        ...
        // ✅ Parse price breakdown from booking page
        let priceMap = null;
        ...
        // Fixed services with individual prices
        if (fixedServiceNames.length > 0) {
            if (priceMap) {
                // ✅ Use exact prices from booking
                ...
                // Fallback: split evenly if no price breakdown
        ...
        // Other Service with quote breakdown
        let quotedTotal = 0;
        if (hasOtherService) {
        ...
        async checkJobStatus() {
            ...
            if (job) {
                ...
                // Update bill amounts for later use
                this.updateBillAmounts(job);
    ...
    try {
        const { error } = await sb
            ...
            .update({
                ...
                status: 'in_progress'  // ✅ Use 'in_progress' (it's in your constraint)
                // Alternative: status: 'assigned' (also in your constraint)
            })
            .eq('id', this.jobId);
    ...
    try {
        const { error } = await sb
            ...
            .update({
                ...
                status: 'cancelled'  // Valid status in your constraint
            })
            .eq('id', this.jobId);
